...
| DEC-C09 | Lote E | noindex en: /gracias/, /aviso-legal/, /privacidad/, /cookies/, 404.html. | **APPROVED — recuento pendiente de comprobar** | PRE-LAUNCH-CHECKLIST | Resuelto en origen el 2026-09-07: se eliminaron `src/pages/lab/` y `servicios/reformas-integrales-lab.astro`, de modo que ya no existe página de laboratorio que filtrar. Con ellas se retiraron el filtro `/lab/` del sitemap, la integración `exclude-lab-in-production` de `astro.config.mjs` y el `Disallow: /lab/` de `robots.txt.ts`. Comprobado sobre el build del 2026-09-07 (74 páginas): aplicando el filtro de `astro.config.mjs` al listado construido resultan **71 URLs**, que coincide con la cifra aprobada, y ninguna de laboratorio. Es un cálculo sobre el build, no la lectura de un `sitemap-index.xml` generado: con `PUBLIC_SITE_INDEXING=false` el sitemap no se emite. |
